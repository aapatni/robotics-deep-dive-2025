...
We’ll assume the fundamentals of robotics (perception, planning, control, generalization, etc.) are understood; readers who want a detailed history of those can find an excellent existing breakdown here.
...
- [Scope](#scope)
- [1) Research & Engineering — Architectures & Systems](#1-research--engineering--architectures--systems)
...
- [Next Steps (restructuring) based on feedback](#next-steps-restructuring-based-on-feedback)

---

## 1) Research & Engineering — Architectures & Systems
...
> “This false nomenclature of “researcher” and “engineer”, which is a thinly-masked way of describing a two-tier engineering system, is being deleted from @xAI today. There are only engineers. Researcher is a relic term from academia.” - Elon
...
> “A large part of Robot Learning is systems research, but it doesn't quite fit the existing paradigms: think of CoRL, RSS, SOSP, and MLSys. It draws from all but is inherently full-stack --- hardware, control, compute, ml algos.” - DanFei Xu
...
Building on π0’s cross-embodiment breakthrough, Physical Intelligence’s HiRobot advanced the approach with hierarchical policies in π0.5; meanwhile, DeepMind’s Gemini Robotics adapted Gemini 2.0, and humanoid systems began translating this template into tangible capability through parallel efforts from  NVIDIA’s Isaac GR00T, Figure’s Helix (in logistics), and 1X’s Redwood (in domestic trials). (FYI: checkout RoboArena for a benchmark across a bunch of the new policies).

**π0 shows the first glimpse of ‘do what I mean’**
...
**HiRobot layers planning on top of action**
...
**From adapting inside homes to generalizing in unseen environments by co-training on heterogeneous data**
...
![hirobot](./images/hirobot.png)

Based on conversations with research engineers, there is significant development of tightly fused VLAs for task-specific robots. Tightly fused implies that vision, language, and action are jointly trained – meaning there is not necessarily a language or vision backbone.
...
**Strong individual components fail without system-level validation**
...
**Learned observers automate evaluation at massive scale**
...
**Simulated success often collapses on real hardware**
...
**Two strategies to close Sim2Real gap: higher fidelity vs. error correction**
...
**World models help robots imagine diverse scenarios**
...
**Internet-scale video pretraining slashes robotics data needs**

V-JEPA by Meta took a different approach to world models. Instead of trying to build a simulation environment for robots, they attempt to encode the world itself into the pre-training phase. Meta demonstrates that you can pretrain a latent action-condition world model over a million hours of broad, curated internet video, then add just 62 hours of unlabeled robot interaction data, and get a model that can plan and execute manipulation tasks in entirely new environments.
...
**Brute forcing scale with industrial tele-operation pipelines**

![Industrial teleoperation pipeline.](images/teleop.png)


The most straightforward strategy for scaling data collection is industrial-grade teleoperation. This approach is similar to how companies like Scale AI / Mercor / Surge provide data labeling as a service – and there are a host of companies which do this for robotics. It involves capturing vast amounts of human-operated robot actions, often through VR or joystick setups. Tesla's Optimus program is a prime example of this philosophy in action, relying on a massive pipeline of human-teleoperated data to bootstrap its core manipulation capabilities. While effective for gathering volume, this approach still requires significant investment in hardware and operator time.

Teleoperation breaks down on real-world tasks because operators lack force feedback and most hardware doesn’t match the exact way humans move (as heard in Dwarkesh’s LabelBox ad, “it’s a bit harder than I anticipated”). This mismatch makes demonstrations slow, error-prone, and biased, while also limiting the diversity of tasks that can be shown. Thus, data pipelines are also bottlenecked at the hardware layer, where small design choices (like whether fingers can spread, feel contact forces, or use fingernails) decide whether you can even capture the right kind of manipulation data at scale.

**Fidelity improves with perioperation, when humans and robots move together**

![DEXOP perioperation](images/dexop.png)

DEXOP introduces perioperation, which involves collecting robot training data by having a person wearing an exoskeleton that mirrors their hand movements onto a robot hand, so the human feels the force and the system records the motion and touch data. By designing the exoskeleton and robot hand, they produced more useful data per unit of time and higher-quality policies than tele-op.

**Humans can be sensors themselves via egocentric data collection**

![EgoZero](images/egozero.png)

A more advanced variant of this is the "human-as-sensor" model, which aims for higher-fidelity data. Instead of just recording joystick commands, companies like Mecha AI use smart glasses (EgoZero) and tactile gloves (Feel the Force) to capture data directly from human sight and sensation, enabling a more nuanced and direct transfer of skills.

**Augmenting Datasets After Collection**
...
- **Deployment setting** – are they deployed in an industrial or consumer setting?
- **System integrators** = FDE
- **Model strategy** – do they train their own foundation model or fine-tune others?
- **Form factor** – what physical body do they build?

**Dynamics**

Open Source, Open Weights, Open Data

In the past 12 months, we’ve seen a surge in capital, demos, and technical results in humanoids, large-scale model releases, and vertical robotics-as-a-service builders, as well as new entrants in both hardware and AI architectures. VCs have been publishing their theses too (Robotics Won’t Have a ChatGPT Moment by Coatue, The Robotics Breakout Moment and Salesforce Ventures Guide to Investing in Robotics by Salesforce Ventures, The Age of Embodied Intelligence by Lux Capital, etc.)
...
**Consumer:** 1X, K-Scale, Hugging Face, The Bot Company, Matic
...
**Industrial:** Tesla Optimus, Figure, Persona, Neura, Apptronik, Gecko Robotics, Locus Robotics, Watney Robotics, Lucid Bots, Formic, Asylon Robotics, Knightscope
...
**Foundation Model Labs:** DeepMind, Physical Intelligence, Skild, WorldLabs, Genesis, Generalist, Dyna, Field, Figure, 1X, RLWRLD, Covariant, General Robotics
...
Some labs may also experiment with “model-as-the-product” strategies. For example, it may be more efficient to buy an unoptimized business with few existing robotics solutions and architect the full process from scratch, rather than “fine-tuning” existing processes around legacy systems.
...
- Why would a frontier lab create a hierarchical policy vs a tightly fused one? Our hypothesis is A tightly fused VLA is good at specializing and a hierarchical one can reason across tasks. Are these tightly fused VLAs small?
- What is the best analogy for scaling in robotics? Deep learning? Computer use? Etc. Where are we on the curve?
- Andrej Karpathy had a short ‘state of the union’ for what matters the most on AI training. He believes environments (like exercises or practice problems for the AI, letting it learn by doing, not just by reading or imitating to some RL reward function) are the most valuable asset for advancing language models
- How do we build a large, diverse set of environments?
- Companies like Prime Intellect are facing this task
- How are each of the frontier labs positioning themselves (PI, Skild, Field, Generalist, etc.)?
- Who is winning RoboArena?
- For folks we talk to - which labs will hit RAGI first?
- Things to track
  - Talent density distribution
  - Robotics MIDAS lists
  - Is there evidence that research is the product?
  - How are model policies evaluated? At Tesla, Pi, general purpose home robotics
  - What corpus of talent will lead to winning in robotics (DL scientists, or a more nuanced mix of deployment and DL researchers)?
  - Are there monotonic scaling laws for $ to model capabilities? What are the scaling laws for robotics: cost/data/computation → capability?
  - To what extent can simulation reduce evaluation variance or accelerate iteration effectively?
  - How do we quantify task complexity systematically rather than intuitively?
  - How do we decide how much data is enough for a given task or model?
  - What infrastructure or tooling would make robotics research significantly easier if built by others? For example, how do we design infrastructure for continuously growing datasets that differ from static LLM datasets?
